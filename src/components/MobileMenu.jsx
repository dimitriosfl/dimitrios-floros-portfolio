...
export default function MobileMenu({ menuOpen, setMenuOpen })
{
    ...
    return (<div className={`fixed top-0 left-0 w-full bg-black/80 z-40 flex flex-col items-center justify-center transition-all duration-300 ease-in-out
         ${menuOpen ? "h-screen opacity-100 pointer-events-auto" : "h-0 opacity-0 pointer-event-none"}`}>
        <button onClick={() => setMenuOpen(false)} className="absolute top-10 right-7.5 text-white text-3xl focus:outline-none cursor-pointer"
